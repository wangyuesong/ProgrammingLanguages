...
case class MyCons[A](x: A, xs: MyList[A]) extends MyList[A] {
  ...
  // Every time just generate one B entry and create a new list with single element
  def map[B](f: A => B): MyList[B] = {
    flatMap((a: A) => MyList(f(a)))
  }

  // Base: MyList[B]()
  // Function: take a base. foldLeft will pass current x 
  def flatMap[B](f: A => MyList[B]): MyList[B] = {
    foldLeft(MyList[B]())((list: MyList[B], entry: A)=> list.append(f(entry)))
  }

  def filter(pred: A => Boolean): MyList[A] = {
    pred(x) match{
      case false => xs.filter(pred)
      case true => MyCons[A](x, xs.filter(pred))
    }
  }

  //Create a new entry each time
  def append(other: MyList[A]): MyList[A] = {
     MyCons[A](x, xs.append(other))
  }
 
  // 1. Takes initial and apply f to it and current element x
  // 2. Use this initial as new base and call foldLeft on rest of the list
  def foldLeft[B](initial: B)(f: (B, A) => B): B = {
    //Compute this f add add it to current initial(base) first
    val base = f(initial, x)
    //Call foldLeft on rest of the list using new computed base
    xs.foldLeft(base)(f)
  }

  def foldRight[B](initial: B)(f: (A, B) => B): B = {
    //Fold right will return a computed base
  ...
  //Don't create new list, simply return different part of this list
...
case class MyNil[A]() extends MyList[A] {
  ...
  def append(other: MyList[A]): MyList[A] = {
    other match{
      case _ if other.length > 0 => MyCons(other.head, other.tail)
      case _ => MyNil[A]()
    }
  }

  def foldLeft[B](initial: B)(f: (B, A) => B): B = initial
  //Empty list, simply return initial
