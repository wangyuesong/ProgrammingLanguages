...
// Each one is a function. E.g ExampleMask.vstrip is a returns a ImgMask(function)
// which taks a point and return true or false
object ExampleMask {
  val vstrip: ImgMask =
    pt => abs(pt.x) <= 0.5
...
object Images {
  ...
  // Img[A] is a function signature, takes a point and return a type of A
  type Img[A] = Point => A
  // Function sig that Takes a point and return a boolean
  type ImgMask = Img[Boolean]
  // Function sig that takes a point and return a value(double)
  ...
  // Explain: Return a function that takes what ever Point and return a fixed value of type A
  // !!FILL ME IN
  // 将一个值返回成take一个Point取的一个值的函数
  ...
  def lift1[A, B](ab: A => B)(a: Img[A]): Img[B] = (p: Point) => ab(a(p))
  ...
  // of type C. 
  // COMMENT: Img of Type C, means Img[C], which basically is a function
  // that like: Point=>C
  // !!FILL ME IN
  // a: Point=>Boolean[type ImgMask] b: Point=>Value[type ImgGray]
  // abc: takes to function and generate another function that might return some
  // type of Image[Point => Something]
  // A,B,C are both generic type which should be a function like Point=> ...
  def lift2[A, B, C](abc: (A, B) => C)(a: Img[A])(b: Img[B]): Img[C] = (p: Point) => abc(a(p), b(p))
  ...
  // !!FILL ME IN
  def lift3[A, B, C, D](abcd: (A, B, C) => D)(a: Img[A])(b: Img[B])(c: Img[C]): Img[D] = (p: Point) => abcd(a(p), b(p), c(p))
  ...
  def lerpImgClr(w: ImgGray)(imgA: ImgClr)(imgB: ImgClr): ImgClr = {
    lift3((v: Value, c1: Colour, c2: Colour) => lerpClr(v, c1, c2))(w)(imgA)(imgB)
  }
  ...
  // !!FILL ME IN
  // lift means overlay two functions to generated function's input
  def overlayImgClr(imgA: ImgClr)(imgB: ImgClr): ImgClr = {
    lift2((c1: Colour, c2: Colour) => overlayClr(c1, c2))(imgA)(imgB)
  }
  ...
  // !!FILL ME IN
  // 根据mask选一个ImgClr
  def selectImgClr(m: ImgMask)(imgA: ImgClr)(imgB: ImgClr): ImgClr = {
    lift3((b: Boolean, c1: Colour, c2: Colour) => if (b) c1 else c2)(m)(imgA)(imgB)
  }
  ...
  // !!FILL ME IN
  // 根据mask选一个全黑或者全白的ImgClr
  def msk2clr(m: ImgMask) = {
    lift1((b: Boolean) => if (b) black else white)(m)
  }
  ...
  // !!FILL ME IN
  def gray2clr(m: ImgGray) = {
    lift1((v: Value) => Colour(v, v, v, 1))(m)
  }
}
...
object SpatialTransforms {
  ...
  def rotatePt(d: Double): TransformPt = {
    (p: Point) => Point(p.x * Math.cos(d) - p.y * Math.sin(d), p.y * Math.cos(d) + p.x * Math.sin(d))
  }
  ...
  // !!FILL ME IN
  def swirlPt(d: Double): TransformPt = {
    (p: Point) => rotatePt(Utility.dist(p) * (2.0d * Math.PI / d))(p)
  }

  // define a method named 'polarXf' that takes a TransformPlr as
  // argument and returns a TransformPt.
  // !!FILL ME IN
  def polarXf(t: TransformPlr): TransformPt = {
    (p: Point) => Utility.plr2pt(t(Utility.pt2plr(p)))
  }
  ...
  def translate[A](v: Vector): Filter[A] = {
    (img: Img[A]) => (p: Point) => img(translatePt(v.neg)(p))
  }
  ...
  def scale[A](v: Vector): Filter[A] = {
    (img: Img[A]) => (p: Point) => img(scalePt(v.inverse)(p))
  }
  ...
  // !!FILL ME IN
  def rotate[A](d: Double): Filter[A] = {
    (img: Img[A]) => (p: Point) => img(rotatePt(-1 * d)(p))
  }
  ...
  // !!FILL ME IN
  def swirl[A](d: Double): Filter[A] = {
    (img: Img[A]) => (p: Point) => img(swirlPt(-1d * d)(p))
  }
}
...
object Masking {
  ...
  // !!FILL ME IN
  def intersectM(m1: ImgMask)(m2: ImgMask): ImgMask = {
    // P => true or false
    (p: Point) => lift2((b1: Boolean, b2: Boolean) => b1 & b2)(m1)(m2)(p)
  }
  ...
  // !!FILL ME IN
  def unionM(m1: ImgMask)(m2: ImgMask): ImgMask = {
    (p: Point) => lift2((b1: Boolean, b2: Boolean) => b1 || b2)(m1)(m2)(p)
  }
  ...
  // !!FILL ME IN
  def xorM(m1: ImgMask)(m2: ImgMask): ImgMask = {
    (p: Point) => lift2((b1: Boolean, b2: Boolean) => b1 ^ b2)(m1)(m2)(p)
  }
  ...
  // !!FILL ME IN
  def diffM(m1: ImgMask)(m2: ImgMask): ImgMask = {
    //    (p: Point) => lift2((b1: Boolean, b2: Boolean) => b1 ^ b2)(m1)(m2)(p)
    // Different in result(xor) and X intersect with TREU result in true
    (p: Point) => xorM(m1)(m2)(p) && intersectM(m1)(fullM)(p)
  }
