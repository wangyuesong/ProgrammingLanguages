...
def addOneToEach(ints: List[Int]): List[Int] = {
  if(ints.isEmpty)  List()
  else (ints.head + 1) ::addOneToEach(ints.tail)
}
...
assert(List(1, 2, 3).map(_ + "") == List("1", "2", "3"))
...
assert(Seq("moo", "cow", "bull").foldRight(List[Int]())((e,acc)=>e.length::acc) == List(3, 3, 4))
