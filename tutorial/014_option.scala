...
randomOption(7).map(_=>14).map(i => assert(i == 14))
randomOption("foo").map(_=>"length").map(s => assert(s.length == 6))
...
assert(obj1.map(i => i + 2).getOrElse(3) == 3)
assert(obj2.map(_=>None).getOrElse(3) == 3)
