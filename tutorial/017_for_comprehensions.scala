...
// Here we use the semantics of Option's map which cascades failure.

// `for` also supports an extended form with more than one
...
// The syntax is a nice addition but adds nothing beyond the
